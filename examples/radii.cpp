...
int main(int argc, char **argv) {
  ...
  long memory_bytes =
      ((argc >= 3) ? atol(argv[2]) : 8l) * (1024l * 1024l * 1024l);
  ...
  graph.set_vertex_data_bytes(graph.vertices *
                              (sizeof(VertexId) + sizeof(long) * 2));
    ...
    active_vertices = graph.stream_edges<VertexId>(
        [&](Edge &e) {
          if (visited[e.target][now] != visited[e.source][now]) {
            __sync_fetch_and_or(&visited[e.target][next],
                                visited[e.source][now]);
  ...
  while (candidates.size() < K) {
    for (VertexId i = 0; i < graph.vertices; i++) {
      if (radii[i] == max_radii - threshold)

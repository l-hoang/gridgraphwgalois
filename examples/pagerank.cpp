...
int main(int argc, char **argv) {
  if (argc < 3) {
    fprintf(stderr,
            "usage: pagerank [path] [iterations] [memory budget in GB]\n");
    exit(-1);
  }
  std::string path = argv[1];
  int iterations = atoi(argv[2]);
  long memory_bytes = (argc >= 4) ? atol(argv[3]) * 1024l * 1024l * 1024l
  ...
  long vertex_data_bytes =
      (long)graph.vertices * (sizeof(VertexId) + sizeof(float) + sizeof(float));
  ...
  for (int iter = 0; iter < iterations; iter++) {
    ...
    if (iter == iterations - 1) {
      graph.stream_vertices<VertexId>(
          ...
          [&](std::pair<VertexId, VertexId> vid_range) { pagerank.save(); });
    } else {

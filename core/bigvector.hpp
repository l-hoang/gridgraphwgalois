...
public:
  ...
  BigVector(std::string path, size_t length) { init(path, length); }
  BigVector(std::string path) { init(path); }
  ~BigVector() {
    ...
    if (file_size(path) != sizeof(T) * length) {
      ...
      for (long offset = 0; offset < file_length;) {
        if (file_length - offset > PAGESIZE) {
          ...
          assert(write(fout, buffer, file_length - offset) ==
                 file_length - offset);
...
#pragma omp parallel num_threads(parallelism)
    {
      ...
      std::tie(begin_i, end_i) = get_partition_range(
          length, omp_get_num_threads(), omp_get_thread_num());
...
#pragma omp barrier
  ...
  void sync() { assert(msync(data, sizeof(T) * length, MS_SYNC) == 0); }
  ...
  void load(size_t begin_i, size_t end_i) {
    ...
    this->end_i = end_i;
    in_memory = true;
    // data_in_memory = (T *)memalign(PAGESIZE, (end_i - begin_i) * sizeof(T) +
    // PAGESIZE);
    // assert(data_in_memory!=NULL);
    data_in_memory =
        (T *)mmap(0, (end_i - begin_i) * sizeof(T) + PAGESIZE,
    ...
    while (offset < end_offset) {
      bytes = pread(fd, data_in_memory + (offset / sizeof(T) - begin_i),
  ...
  void save() {
    ...
    long bytes;
    while (offset < end_offset) {
      bytes = pwrite(fd, data_in_memory + (offset / sizeof(T) - begin_i),
                     (end_offset - offset + PAGESIZE - 1) / PAGESIZE * PAGESIZE,
                     offset);
      if (bytes == -1) {
        printf("%ld %ld\n", offset, end_offset);
